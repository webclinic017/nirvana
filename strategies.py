...
class Nirvana(bt.Strategy):
    ...
    def next(self):
        ...
        if 'SPY' in self.target:
            if (spy_price < spy_sma * 0.95 and self.spy_sma_above):
                self.target_update['SPY'] = 0
                rebalance_sma = True
                self.spy_sma_above = False
            elif (spy_price >= spy_sma * 1.04 and not self.spy_sma_above):
                self.target_update['SPY'] = self.target['SPY']
                rebalance_sma = True
                self.spy_sma_above = True
        if 'SPXL' in self.target:
            if (spxl_price < spxl_sma * 0.90 and self.spxl_sma_above):
                self.target_update['SPXL'] = 0
                rebalance_sma = True
                self.spxl_sma_above = False
            elif (spxl_price >= spxl_sma * 1.05 and not self.spxl_sma_above):
                self.target_update['SPXL'] = self.target['SPXL']
                rebalance_sma = True
                self.spxl_sma_above = True
        if 'TQQQ' in self.target:
            if (tqqq_price < tqqq_sma * 0.95 and self.tqqq_sma_above):
                self.target_update['TQQQ'] = 0
                rebalance_sma = True
                self.tqqq_sma_above = False
            elif (tqqq_price >= tqqq_sma * 1.03 and not self.tqqq_sma_above):
                self.target_update['TQQQ'] = self.target['TQQQ']
                rebalance_sma = True
                self.tqqq_sma_above = True
        if 'TLT' in self.target:
            if (tlt_price < tlt_sma * 0.90 and self.tlt_sma_above):
                self.target_update['TLT'] = 0
                rebalance_sma = True
                self.tlt_sma_above = False
            elif (tlt_price >= tlt_sma * 1.05 and not self.tlt_sma_above):
                self.target_update['TLT'] = self.target['TLT']
                rebalance_sma = True
                self.tlt_sma_above = True
        if 'TMF' in self.target:
            if (tmf_price < tmf_sma * 0.90 and self.tmf_sma_above):
                self.target_update['TMF'] = 0
                rebalance_sma = True
                self.tmf_sma_above = False
            elif (tmf_price >= tmf_sma * 1.05 and not self.tmf_sma_above):
                self.target_update['TMF'] = self.target['TMF']
                rebalance_sma = True
                self.tmf_sma_above = True
        if 'GBTC' in self.target:
            if (gbtc_price < gbtc_sma * 0.9 and self.gbtc_sma_above):
                self.target_update['GBTC'] = 0
                rebalance_sma = True
                self.gbtc_sma_above = False
            elif (gbtc_price >= gbtc_sma * 1.05 and not self.gbtc_sma_above):
                self.target_update['GBTC'] = self.target['GBTC']
                rebalance_sma = True
                self.gbtc_sma_above = True
        if 'ETHE' in self.target:
            if (ethe_price < ethe_sma * 0.9 and self.ethe_sma_above):
                self.target_update['ETHE'] = 0
                rebalance_sma = True
                self.ethe_sma_above = False
            elif (ethe_price >= ethe_sma * 1.05 and not self.ethe_sma_above):
                self.target_update['ETHE'] = self.target['ETHE']
                rebalance_sma = True
                self.ethe_sma_above = True
        if 'GLD' in self.target:
            if (gld_price < gld_sma * 0.9 and self.gld_sma_above):
                self.target_update['GLD'] = 0
                rebalance_sma = True
                self.gld_sma_above = False
            elif (gld_price >= gld_sma * 1.05 and not self.gld_sma_above):
                self.target_update['GLD'] = self.target['GLD']
                rebalance_sma = True
                self.gld_sma_above = True
        if 'UGL' in self.target:
            if (ugl_price < ugl_sma * 0.9 and self.ugl_sma_above):
                self.target_update['UGL'] = 0
                rebalance_sma = True
                self.ugl_sma_above = False
            elif (ugl_price >= ugl_sma * 1.05 and not self.ugl_sma_above):
                self.target_update['UGL'] = self.target['UGL']
                rebalance_sma = True
                self.ugl_sma_above = True
        if 'TSLA' in self.target:
            if (tsla_price < tsla_sma * 0.90 and self.tsla_sma_above):
                self.target_update['TSLA'] = 0
                rebalance_sma = True
                self.tsla_sma_above = False
            elif (tsla_price >= tsla_sma * 1.05 and not self.tsla_sma_above):
